...
In this way, you'll get those returns:
...
(By the way, `rest` can be call as you prefer as long as you use `...` before).
...
A little note, be careful with this way because using `slice(-1)` keeps you only the last item, so:
